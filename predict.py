...
#    lidar_file = 'C:/Lidar_kittiDataset/Training/velodyne/004653.bin'  # path to one lidar file with an indice from idx (e.g. Path/to/file/velodyne/003001.bin)
#    calibration_file = 'C:/Lidar_kittiDataset/Training/calib/004653.txt'# path to one calibration file with an indice from idx (e.g. Path/to/file/calib/003001.txt)
#    label_file = 'C:/Lidar_kittiDataset/Training/label_2/004653.txt'
...
#        print("Box predicted!") 
        ...
        corners_predicted.append(corners_pre)
...
#    utils.cp_and_avg_precision(target,all_boxes,idx,result_matrix)
    utils.iou_avg_precision(all_corners,corners_predicted,idx,result_matrix,all_boxes)
...
final_predicted_matrix.insert(0,['Image id','predicted_box_index','score','TP','FP','acc TP','acc FP','precision','recall'])
precision_list=[]
recall_list=[]
for i in range(1,len(final_predicted_matrix)):
    print(final_predicted_matrix[i-1])
...
def precision_interpolated(prec_list, rec_list, cur_recall):
    ...
    for i in range(len(rec_list)):
        if(rec_list[i] <= cur_recall):
            greater_recall = greater_recall+1
    if greater_recall == 0:
        return 0
    
    return prec_list[greater_recall-1]
...
AP11 = 1/11 * sum(precision_interpolated_list)

print('AP11',AP11)'''

print(final_predicted_matrix[len(final_predicted_matrix)-1])
...
print('number_groundtruth',number_groundtruth)
